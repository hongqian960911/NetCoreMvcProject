...
},beforePageText:"第",afterPageText:"/ {pages} 页",displayMsg:"显示 {from} 到 {to} 条 / 共 {total} 条",nav:{first:{iconCls:"pagination-first",handler:function(){
...
$.fn.form.defaults={fieldTypes:["tagbox","combobox","combotree","combogrid","combotreegrid","datetimebox","datebox","combo","datetimespinner","timespinner","numberspinner","spinner","slider","searchbox","numberbox","passwordbox","filebox","textbox","switchbutton","radiobutton","checkbox"],novalidate:false,ajax:true,iframe:true,dirty:false,dirtyFields:[],url:null,queryParams:{},onSubmit:function(_672){
...
(function($){
function init(_cbc){
var _cbd=$("<div class=\"slider\">"+"<div class=\"slider-inner\">"+"<a href=\"javascript:;\" class=\"slider-handle\"></a>"+"<span class=\"slider-tip\"></span>"+"</div>"+"<div class=\"slider-rule\"></div>"+"<div class=\"slider-rulelabel\"></div>"+"<div style=\"clear:both\"></div>"+"<input type=\"hidden\" class=\"slider-value\">"+"</div>").insertAfter(_cbc);
var t=$(_cbc);
t.addClass("slider-f").hide();
var name=t.attr("name");
if(name){
_cbd.find("input.slider-value").attr("name",name);
t.removeAttr("name").attr("sliderName",name);
}
_cbd._bind("_resize",function(e,_cbe){
if($(this).hasClass("easyui-fluid")||_cbe){
_cbf(_cbc);
}
return false;
});
...
if(_cc1){
if(_cc1.width){
opts.width=_cc1.width;
}
if(_cc1.height){
opts.height=_cc1.height;
}
}
_cc3._size(opts);
if(opts.mode=="h"){
_cc3.css("height","");
_cc3.children("div").css("height","");
}else{
_cc3.css("width","");
_cc3.children("div").css("width","");
_cc3.children("div.slider-rule,div.slider-rulelabel,div.slider-inner")._outerHeight(_cc3._outerHeight());
}
_cc4(_cc0);
};
function _cc5(_cc6){
var _cc7=$.data(_cc6,"slider");
...
_cc9(aa);
function _cc9(aa){
var rule=_cc8.find("div.slider-rule");
var _cca=_cc8.find("div.slider-rulelabel");
rule.empty();
_cca.empty();
for(var i=0;i<aa.length;i++){
var _ccb=i*100/(aa.length-1)+"%";
var span=$("<span></span>").appendTo(rule);
span.css((opts.mode=="h"?"left":"top"),_ccb);
if(aa[i]!="|"){
span=$("<span></span>").appendTo(_cca);
span.html(aa[i]);
if(opts.mode=="h"){
span.css({left:_ccb,marginLeft:-Math.round(span.outerWidth()/2)});
}else{
span.css({top:_ccb,marginTop:-Math.round(span.outerHeight()/2)});
}
}
}
};
...
_ccf.removeClass("slider-h slider-v slider-disabled");
_ccf.addClass(opts.mode=="h"?"slider-h":"slider-v");
_ccf.addClass(opts.disabled?"slider-disabled":"");
var _cd0=_ccf.find(".slider-inner");
_cd0.html("<a href=\"javascript:;\" class=\"slider-handle\"></a>"+"<span class=\"slider-tip\"></span>");
if(opts.range){
_cd0.append("<a href=\"javascript:;\" class=\"slider-handle\"></a>"+"<span class=\"slider-tip\"></span>");
}
_ccf.find("a.slider-handle").draggable({axis:opts.mode,cursor:"pointer",disabled:opts.disabled,onDrag:function(e){
...
_cce.isDragging=true;
opts.onSlideStart.call(_ccd,opts.value);
},onStopDrag:function(e){
_cd2(opts.mode=="h"?e.data.left:e.data.top,this);
opts.onSlideEnd.call(_ccd,opts.value);
opts.onComplete.call(_ccd,opts.value);
_cce.isDragging=false;
}});
_ccf.find("div.slider-inner")._unbind(".slider")._bind("mousedown.slider",function(e){
if(_cce.isDragging||opts.disabled){
return;
}
var pos=$(this).offset();
_cd2(opts.mode=="h"?(e.pageX-pos.left):(e.pageY-pos.top));
opts.onComplete.call(_ccd,opts.value);
});
function _cd3(_cd4){
var dd=String(opts.step).split(".");
var dlen=dd.length>1?dd[1].length:0;
return parseFloat(_cd4.toFixed(dlen));
};
function _cd2(pos,_cd5){
var _cd6=_cd7(_ccd,pos);
var s=Math.abs(_cd6%opts.step);
if(s<opts.step/2){
_cd6-=s;
}else{
_cd6=_cd6-s+opts.step;
}
_cd6=_cd3(_cd6);
if(opts.range){
var v1=opts.value[0];
...
if(_cd5){
var _cd8=$(_cd5).nextAll(".slider-handle").length>0;
if(_cd6<=v2&&_cd8){
v1=_cd6;
}else{
if(_cd6>=v1&&(!_cd8)){
...
_cd6<m?v1=_cd6:v2=_cd6;
}
}
}
$(_ccd).slider("setValues",[v1,v2]);
}else{
$(_ccd).slider("setValue",_cd6);
}
};
};
function _cd9(_cda,_cdb){
var _cdc=$.data(_cda,"slider");
var opts=_cdc.options;
var _cdd=_cdc.slider;
var _cde=$.isArray(opts.value)?opts.value:[opts.value];
var _cdf=[];
if(!$.isArray(_cdb)){
_cdb=$.map(String(_cdb).split(opts.separator),function(v){
return parseFloat(v);
});
}
_cdd.find(".slider-value").remove();
var name=$(_cda).attr("sliderName")||"";
...
var _ce1=$("<input type=\"hidden\" class=\"slider-value\">").appendTo(_cdd);
_ce1.attr("name",name);
_ce1.val(_ce0);
_cdf.push(_ce0);
var _ce2=_cdd.find(".slider-handle:eq("+i+")");
var tip=_ce2.next();
var pos=_ce3(_cda,_ce0);
if(opts.showTip){
tip.show();
tip.html(opts.tipFormatter.call(_cda,_ce0));
}else{
tip.hide();
}
if(opts.mode=="h"){
var _ce4="left:"+pos+"px;";
_ce2.attr("style",_ce4);
tip.attr("style",_ce4+"margin-left:"+(-Math.round(tip.outerWidth()/2))+"px");
}else{
var _ce4="top:"+pos+"px;";
_ce2.attr("style",_ce4);
tip.attr("style",_ce4+"margin-left:"+(-Math.round(tip.outerWidth()))+"px");
}
}
opts.value=opts.range?_cdf:_cdf[0];
$(_cda).val(opts.range?_cdf.join(opts.separator):_cdf[0]);
if(_cde.join(",")!=_cdf.join(",")){
opts.onChange.call(_cda,opts.value,(opts.range?_cde:_cde[0]));
}
};
function _cc4(_ce5){
var opts=$.data(_ce5,"slider").options;
var fn=opts.onChange;
opts.onChange=function(){
};
_cd9(_ce5,opts.value);
opts.onChange=fn;
};
function _ce3(_ce6,_ce7){
var _ce8=$.data(_ce6,"slider");
var opts=_ce8.options;
var _ce9=_ce8.slider;
var size=opts.mode=="h"?_ce9.width():_ce9.height();
var pos=opts.converter.toPosition.call(_ce6,_ce7,size);
if(opts.mode=="v"){
pos=_ce9.height()-pos;
}
if(opts.reversed){
...
function _cd7(_cea,pos){
var _ceb=$.data(_cea,"slider");
var opts=_ceb.options;
var _cec=_ceb.slider;
var size=opts.mode=="h"?_cec.width():_cec.height();
var pos=opts.mode=="h"?(opts.reversed?(size-pos):pos):(opts.reversed?pos:(size-pos));
var _ced=opts.converter.toValue.call(_cea,pos,size);
return _ced;
};
$.fn.slider=function(_cee,_cef){
if(typeof _cee=="string"){
return $.fn.slider.methods[_cee](this,_cef);
}
_cee=_cee||{};
return this.each(function(){
var _cf0=$.data(this,"slider");
if(_cf0){
$.extend(_cf0.options,_cee);
}else{
_cf0=$.data(this,"slider",{options:$.extend({},$.fn.slider.defaults,$.fn.slider.parseOptions(this),_cee),slider:init(this)});
$(this)._propAttr("disabled",false);
}
var opts=_cf0.options;
...
_cd9(this,opts.range?[opts.min,opts.max]:[opts.min]);
});
},reset:function(jq){
return jq.each(function(){
var opts=$(this).slider("options");
...
$.fn.slider.parseOptions=function(_cf4){
var t=$(_cf4);
return $.extend({},$.parser.parseOptions(_cf4,["width","height","mode",{reversed:"boolean",showTip:"boolean",range:"boolean",min:"number",max:"number",step:"number"}]),{value:(t.val()||undefined),disabled:(t.attr("disabled")?true:undefined),rule:(t.attr("rule")?eval(t.attr("rule")):undefined)});
};
$.fn.slider.defaults={width:"auto",height:"auto",mode:"h",reversed:false,showTip:false,disabled:false,range:false,value:0,separator:",",min:0,max:100,step:1,rule:[],tipFormatter:function(_cf5){
return _cf5;
},converter:{toPosition:function(_cf6,size){
var opts=$(this).slider("options");
var p=(_cf6-opts.min)/(opts.max-opts.min)*size;
return p;
},toValue:function(pos,size){
var opts=$(this).slider("options");
var v=opts.min+(opts.max-opts.min)*(pos/size);
return v;
}},onChange:function(_cf7,_cf8){
},onSlideStart:function(_cf9){
},onSlideEnd:function(_cfa){
},onComplete:function(_cfb){
}};
})(jQuery);
